...
export default function AboutPage() {
  return (
    <div style={{background:'linear-gradient(180deg,#dce8f2 0%,#e8f0f8 50%,#dce8f2 100%)',minHeight:'100vh'}}>
      ...
      <div style={{background:'linear-gradient(90deg,#2a4a6b,#3d6490)'}}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {NUMBERS.map((n) => (
              <div key={n.label} style={{textAlign:'center'}}>
                ...
                <div style={{color:'rgba(255,255,255,0.80)',fontSize:'15px',fontWeight:600,marginTop:'6px'}}>{n.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{background:'linear-gradient(135deg,#0f2238 0%,#1a3a5c 100%)',position:'relative',overflow:'hidden'}}>
        <div style={{position:'absolute',inset:0,background:'radial-gradient(ellipse at 30% 50%,rgba(33,100,180,0.15) 0%,transparent 70%)'}} />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20" style={{position:'relative',zIndex:1}}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div>
              ...
              <p style={{color:'rgba(255,255,255,0.85)',fontSize:'16px',lineHeight:1.8,fontWeight:500}}>Каждое решение принимается на основе опыта врача, актуальных научных данных и индивидуальных особенностей пациента.</p>
            </div>
            <div>
              <h3 style={{fontFamily:'var(--font-playfair)',fontSize:'20px',fontWeight:700,color:'#ffffff',marginBottom:'12px'}}>Открытость</h3>
              <p style={{color:'rgba(255,255,255,0.85)',fontSize:'16px',lineHeight:1.8,fontWeight:500}}>Мы подробно объясняем результаты обследований, предлагаем понятные варианты лечения и отвечаем на все вопросы.</p>
            </div>
            <div>
              <h3 style={{fontFamily:'var(--font-playfair)',fontSize:'20px',fontWeight:700,color:'#ffffff',marginBottom:'12px'}}>Забота</h3>
              <p style={{color:'rgba(255,255,255,0.85)',fontSize:'16px',lineHeight:1.8,fontWeight:500}}>Нам важны не только медицинские результаты, но и психологический комфорт каждого пациента.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
          {DIRECTIONS.map((dir, i) => (
            <div key={dir.name} style={{borderTop:'1px solid rgba(33,100,180,0.25)',padding:'28px 0',display:'flex',alignItems:'flex-start',gap:'24px'}}>
              <span style={{fontFamily:'var(--font-playfair)',fontSize:'32px',fontWeight:800,color:'rgba(33,100,180,0.25)',lineHeight:1,minWidth:'48px'}}>0{i+1}</span>
              <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-x-12 gap-y-10">
            {PRINCIPLES.map((item) => (
              <div key={item.title} style={{display:'flex',alignItems:'flex-start',gap:'16px'}}>
                ...
                <h3 style={{fontSize:'17px',fontWeight:700,color:'#ffffff',paddingTop:'8px'}}>{item.title}</h3>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{background:'linear-gradient(135deg,#0f2238 0%,#1a3a5c 100%)',position:'relative',overflow:'hidden'}}>
        <div style={{position:'absolute',inset:0,background:'radial-gradient(ellipse at 50% 50%,rgba(33,100,180,0.20) 0%,transparent 70%)'}} />
